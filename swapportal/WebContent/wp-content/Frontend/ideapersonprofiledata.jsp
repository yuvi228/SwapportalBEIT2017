...
<%
List<Ideaperson> ideapersonlist = (List<Ideaperson>) session.getAttribute("Ideaperson");
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Ideaperson Profile Details</title>
...
<body background="<%=request.getContextPath()%>/Image/bg.jpg">
	...
	<div class="container shortcodes">
		<div class="row">
			<div class="col-sm-12">
				<h4 class="margin30">Ideaperson table data</h4>
				<font color="black">
					<table class="table table-condensed">
						...
						<tbody>
							<%
							if (!ideapersonlist.isEmpty() && ideapersonlist != null) {
								Iterator<Ideaperson> it = ideapersonlist.iterator();
								Ideaperson batchObj = null;
								while (it.hasNext()) {
									batchObj = (Ideaperson) it.next();
							%>
							<tr>
								<td><%=batchObj.getIpid()%></td>
								<td><%=batchObj.getId()%></td>
								<td><%=batchObj.getType()%></td>
								<td><%=batchObj.getAboutyou()%></td>
								<td><%=batchObj.getExpectfund()%></td>
								<td><%=batchObj.getProjectabstract()%></td>
